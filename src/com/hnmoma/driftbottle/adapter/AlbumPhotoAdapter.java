...
public class AlbumPhotoAdapter extends BaseAdapter {
	...
	public AlbumPhotoAdapter(ImageLoader imageLoader, int identityflag) {
...
//		.displayer(new RoundedBitmapDisplayer(2))
	...
	public AlbumPhotoAdapter(ImageLoader imageLoader) {
		this.imageLoader = imageLoader;
		mInfos = new LinkedList<PhotoWallModel>();
...
//		.displayer(new RoundedBitmapDisplayer(2))
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = inflater.inflate(R.layout.griditem_photo, null);
			holder = new Holder();
...
//		if(visitor.getPicId().equals("0000")){
//				holder.iv_photo.setImageResource(R.drawable.vzone_addphoto);
//		}else{
//			MoMaLog.e("debug",visitor.getPicUrl());
//			imageLoader.displayImage(visitor.getPicUrl(), holder.iv_photo, options);
//		}
		
		//第一行和最后一行，需要有间距，每一行有3个
		if(position/3==0){	//第一行
			convertView.setPadding(0, 10, 0, 0);
		}else if(position>=(getCount()/3+getCount()%3==0?0:1-1)*3){ //最后一行
