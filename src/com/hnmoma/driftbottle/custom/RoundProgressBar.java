...
public class RoundProgressBar extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		if(textIsDisplayable && percent != 0 && style == STROKE){
			if(percent<100){
				canvas.drawText(String.valueOf(percent) , centre*0.8f- textWidth / 2, centre*0.95f + textSize/2, paint); //画出进度百分比
				paint.setTextSize(textSize-4);
				float tmp = textWidth;
				textWidth = paint.measureText("%");
				canvas.drawText("%", centre*0.8f+tmp - textWidth / 2, centre*0.95f+ textSize/2, paint); //画出进度百分比
			}
		}
